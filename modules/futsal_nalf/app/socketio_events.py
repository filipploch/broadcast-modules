...
def _get_timer_manager_or_error():
    ...
    if not tm:
        socketio.emit('error', {'message': 'Timer manager not available'})
    return tm
...
def handle_timer_resume(data):
    ...
    socketio.emit('timer_resumed', {'timer_id': timer_id}, broadcast=True)
...
def handle_timer_remove(data):
    ...
    if gt is None or gt.timer_type == GameTimer.TYPE_MAIN:
        socketio.emit('error', {'message': 'Cannot remove main timer or timer not found'})
        return
    ...
    if tm.remove_timer(timer_id):
        ...
        socketio.emit('reload_penalty_timers', {'penalties': penalties}, broadcast=True)
...
def handle_timer_adjust(data):
    ...
    if tm.adjust_time(timer_id, delta):
        ...
        socketio.emit('timer_adjusted', {'timer_id': timer_id, 'delta': delta},
             broadcast=True)
    else:
        socketio.emit('error', {'message': f'Failed to adjust timer {timer_id}'})
...
def handle_timer_set_time(data):
    ...
    if tm.set_elapsed_time(timer_id, elapsed_time):
        ...
        socketio.emit('error', {'message': f'Failed to set time for timer {timer_id}'})
