...
def get_event_camera_model():
    """Zwraca klasę EventCamera zarejestrowaną przez aktywny moduł."""
    from core.extensions import db
    for mapper in db.Model.registry.mappers:
        cls = mapper.class_
        if (getattr(cls, '__tablename__', None) == 'event_cameras'
                and issubclass(cls, BaseEventCameraMixin)):
            return cls
    raise RuntimeError(
        "Nie znaleziono klasy EventCamera w rejestrze SQLAlchemy. "
