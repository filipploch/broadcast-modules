/**
 * time-display.js — wspólny algorytm wyświetlania czasu (KANONICZNA WERSJA).
 *
 * Używany zarówno przez overlaye (hub/overlays/*\/js/time-display.js — kopie
 * tego pliku, synchronizowane ręcznie, bo overlaye i UI są serwowane przez
 * dwa różne serwery bez wspólnego build-stepu) jak i przez UI operatora
 * (ten plik, ładowany przez ui-jinja.html/index.html).
 *
 ...
 * pełną sekundę — spójnie z dotychczasowym zachowaniem UI i
 * FutsalFormatters.formatElapsedTime/formatRemaining.
 *
 * UWAGA: przy zmianie tego pliku zaktualizuj też kopie w:
 *   - hub/overlays/futsal-nalf/js/time-display.js
 *   - hub/overlays/garbarnia/js/time-display.js
 */
(function (global) {
    'use strict';
